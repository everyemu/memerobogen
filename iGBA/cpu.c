...
u32 function_cc step_debug(u32 pc, u32 cycles)
{
  ...
  if(thumb)
  {
    fprintf(stderr, "Thumb instruction at PC: %04x \n",
            read_memory16(reg[REG_PC]));
  }
  else
  {
    fprintf(stderr, "ARM instruction at PC: %08x \n",
            read_memory32(reg[REG_PC]));
  }
#endif
...
#if defined(GP2X_BUILD) && !defined(WITH_INTERPRETER)
  rom_translation_cache = (unsigned char *)(((unsigned long) rom_translation_cache_static + CODE_CACHE_ALIGN_MASK) & ~(CODE_CACHE_ALIGN_MASK));
  if((rom_translation_cache = mmap(rom_translation_cache, ROM_TRANSLATION_CACHE_SIZE, PROT_READ|PROT_WRITE|PROT_EXEC, MAP_ANON|MAP_FIXED|MAP_SHARED, -1, 0)) == MAP_FAILED)
    fprintf(stderr, "Couldn't mmap rom err: 0x%x! \n", errno);
	//if (mprotect(rom_translation_cache, ROM_TRANSLATION_CACHE_SIZE, PROT_READ|PROT_WRITE|PROT_EXEC))
	//	fprintf(stderr, "Couldn't mprotect memory! \n");

  ram_translation_cache = (unsigned char *)(((unsigned long) ram_translation_cache_static + CODE_CACHE_ALIGN_MASK) & ~(CODE_CACHE_ALIGN_MASK));
  if((ram_translation_cache = mmap(ram_translation_cache, RAM_TRANSLATION_CACHE_SIZE, PROT_READ|PROT_WRITE|PROT_EXEC, MAP_ANON|MAP_FIXED|MAP_SHARED, -1, 0)) == MAP_FAILED)
    fprintf(stderr, "Couldn't mmap ram err: 0x%x! \n", errno);
	//if (mprotect(ram_translation_cache, RAM_TRANSLATION_CACHE_SIZE, PROT_READ|PROT_WRITE|PROT_EXEC))
	//	fprintf(stderr, "Couldn't mprotect memory! \n");
      
  bios_translation_cache = (unsigned char *)(((unsigned long) bios_translation_cache_static + CODE_CACHE_ALIGN_MASK) & ~(CODE_CACHE_ALIGN_MASK));
  if((bios_translation_cache = mmap(bios_translation_cache, BIOS_TRANSLATION_CACHE_SIZE, PROT_READ|PROT_WRITE|PROT_EXEC, MAP_ANON|MAP_FIXED|MAP_SHARED, -1, 0)) == MAP_FAILED)
    fprintf(stderr, "Couldn't mmap bios err: 0x%x! \n", errno);
	//if (mprotect(bios_translation_cache, BIOS_TRANSLATION_CACHE_SIZE, PROT_READ|PROT_WRITE|PROT_EXEC))
	//	fprintf(stderr, "Couldn't mprotect memory! \n");

  //rom_translation_cache = (unsigned char *)(((unsigned long) rom_translation_cache_static + TARGET_PAGESIZE) & ~(TARGET_PAGESIZE));
	//ram_translation_cache = (unsigned char *)(((unsigned long) ram_translation_cache_static + TARGET_PAGESIZE) & ~(TARGET_PAGESIZE));
	//bios_translation_cache = (unsigned char *)(((unsigned long) bios_translation_cache_static + TARGET_PAGESIZE) & ~(TARGET_PAGESIZE));
  fprintf(stderr, "rom_translation_cache 0x%x rom_translation_cache_static 0x%x ram_translation_cache 0x%x ram_translation_cache_static 0x%x bios_translation_cache 0x%x bios_translation_cache_static 0x%x write_memory32 0x%x \n", rom_translation_cache, rom_translation_cache_static, ram_translation_cache, ram_translation_cache_static, bios_translation_cache, bios_translation_cache_static, write_memory32);
#else
#if !defined(WITH_INTERPRETER)
  rom_translation_cache = rom_translation_cache_static;
  ram_translation_cache = ram_translation_cache_static;
  bios_translation_cache = bios_translation_cache_static;    
#endif
#endif
...
cpu_savestate_builder(read_mem);
