...
/*
u8 rom_translation_cache_static[ROM_TRANSLATION_CACHE_SIZE + 4096];
u8 ram_translation_cache_static[RAM_TRANSLATION_CACHE_SIZE + 4096];
u8 bios_translation_cache_static[BIOS_TRANSLATION_CACHE_SIZE + 4096];
*/
...
u32 (*prepare_load_reg)(u32,u32);
u32 (*prepare_load_reg_pc)(u32,u32,u32);
u32 (*prepare_store_reg)(u32,u32);
void (*complete_store_reg)(u32,u32);
void (*generate_load_reg)(u32,u32);
void (*generate_store_reg)(u32,u32);
...
static u8 *old_rom_translation_ptr;
static u8 *old_ram_translation_ptr;
static u8 *old_bios_translation_ptr;

#define invalidate_dcache(type) \
  if (type##_translation_ptr > old_##type##_translation_ptr) { \
		CLEAR_INSN_CACHE(old_##type##_translation_ptr, (u32) type##_translation_ptr - (u32) old_##type##_translation_ptr); \
		type##_translation_ptr = (char *) \
				(((unsigned int) type##_translation_ptr + \
						CODE_CACHE_ALIGN_MASK) & ~CODE_CACHE_ALIGN_MASK); \
		old_##type##_translation_ptr = type##_translation_ptr; \
	}

static void translate_invalidate_dcache()
{
  /*
	static int drain_wb[16];

	invalidate_dcache(rom);
	invalidate_dcache(ram);
	invalidate_dcache(bios);

	memset(drain_wb, 0, 16*4);
	CLEAR_INSN_CACHE(drain_wb, drain_wb + 16);
	*/
}
...
#define translate_arm_instruction()                                           \
  check_pc_region(g_pc);                                                        \
  opcode = address32(pc_address_block, (g_pc & 0x7FFF));                        \
...
#define translate_thumb_instruction()                                         \
  ...
  check_pc_region(g_pc);                                                        \
  last_opcode = opcode;                                                       \
  opcode = address16(pc_address_block, (g_pc & 0x7FFF));                        \
                                                                              \
...
#define read_block_address_arm()                                              \
  if (((u32) block_address) & 1)                                              \
    block_address = NULL                                                      \

#define read_block_address_thumb()                                            \
  if ((((u32) block_address) & 1) == 0)                                       \
    block_address = NULL                                                      \

#define read_block_address_dual()                                             \
  if (thumb) {                                                                \
    read_block_address_thumb();                                               \
  } else {                                                                    \
    read_block_address_arm();                                                 \
  }                                                                           \

#define write_block_address_arm()                                             \

#define write_block_address_thumb()                                           \
  block_address = (u8 *) (((u32) block_address) | 1)                          \

#define write_block_address_dual()                                            \
  if (thumb)                                                                  \
    write_block_address_thumb()                                               \
...
#define block_lookup_translate(instruction_type, mem_type, smc_enable)        \
  ...
  read_block_address_##instruction_type();                                    \
                                                                              \
  ...
  {                                                                           \
	...
	write_block_address_##instruction_type();                                 \
...
{                                                                             \
  ...
  {                                                                           \
    ...
    default:                                                                  \
      ...
      if(translation_recursion_level == 0)                                    \
      {                                                                       \
        fprintf(stderr, "bad jump %x (%x) (%x)\n", pc, reg[REG_PC], last_instruction); fflush(stderr);     \
        quit();                                                               \
      }                                                                       \
      block_address = (u8 *)(-1);                                             \
      break;                                                                  \
                                                                              ...
                                                                              \
  /*fprintf(stderr, "new block %x (%x)\n", pc, reg[REG_PC]); fflush(stderr);*/\
...
#define arm_base_cycles()                                                     \
  cycle_count += waitstate_cycles_sequential[g_pc >> 24][2]                     \
...
#define thumb_base_cycles()                                                   \
  cycle_count += waitstate_cycles_sequential[g_pc >> 24][1]                     \
...
block_data_type block_data[MAX_BLOCK_SIZE + 1];
block_exit_type block_exits[MAX_EXITS + 1];
...
#define arm_opcode_incomplete(opcode) 0

#define thumb_opcode_incomplete(opcode)                                       \
  ((opcode) >= 0xF000 && (opcode) < 0xF800)                                   \

#define scan_block(type, smc_write_op)                                        \
{                                                                             \
  __label__ block_start;                                                      \
  __label__ block_end;                                                        \
...
block_start:                                                                  \
    ...
    {                                                                         \
      break;                                                                  \
                                                                              ...
                                                                              \
block_end:                                                                    \
  if (type##_opcode_incomplete(opcode))                                       \
    goto block_start;                                                         \
}                                                                             \
...
{                                                                             \
  ...
  u8 *cache_translation_ptr;                                                  \
  u8 *cache_end_translation_ptr;                                              \
  ...
  cycle_count = 0;                                                        \
  g_pc = pc;                                                                  \
  prepare_load_reg = prepare_load_reg_##type;                                 \
  prepare_load_reg_pc = prepare_load_reg_pc_##type;                           \
  prepare_store_reg = prepare_store_reg_##type;                               \
  complete_store_reg = complete_store_reg_##type;                             \
  generate_load_reg = generate_load_reg_##type;                               \
  generate_store_reg = generate_store_reg_##type;                             \
  type##_fix_pc();                                                            \
                                                                              \
  if(pc_address_block == NULL)                                                \
    pc_address_block = load_gamepak_page(pc_region & 0x3FF);                  \
  ...
  {                                                                           \
    case TRANSLATION_REGION_RAM:                                              \
	  if(g_pc >= 0x3000000)                                                       \
      {                                                                       \
        if((g_pc < iwram_code_min) || (iwram_code_min == 0xFFFFFFFF))           \
          iwram_code_min = g_pc;                                                \
      }                                                                       \
      else                                                                    \
                                                                              \
      if(g_pc >= 0x2000000)                                                     \
      {                                                                       \
        if((g_pc < ewram_code_min) || (ewram_code_min == 0xFFFFFFFF))           \
          ewram_code_min = g_pc;                                                \
      }                                                                       \
                                                                              \
    ...
    case TRANSLATION_REGION_BIOS:                                             \
      ...
      break;                                                                  \
                                                                              ...
                                                                              \
  cache_translation_ptr = translation_ptr;                                    \
                                                                              \
  ...
  {                                                                           \
    case TRANSLATION_REGION_RAM:                                              \
      if(g_pc >= 0x3000000)                                                     \
      {                                                                       \
        if((g_pc > iwram_code_max) || (iwram_code_max == 0xFFFFFFFF))           \
          iwram_code_max = g_pc;                                                \
      }                                                                       \
      else                                                                    \
                                                                              \
      if(g_pc >= 0x2000000)                                                     \
      {                                                                       \
        if((g_pc > ewram_code_max) || (ewram_code_max == 0xFFFFFFFF))           \
          ewram_code_max = g_pc;                                                \
      }                                                                       \
                                                                              \
    ...
    case TRANSLATION_REGION_BIOS:                                             \
      ...
      break;                                                                  \
                                                                              ...
                                                                              \
  cache_end_translation_ptr = translation_ptr;                                \
                                                                              \
  ...
  {                                                                           \
    ...
    external_block_exits[i].branch_source, translation_target);               \
                                                                              ...
                                                                              \
  CLEAR_INSN_CACHE(cache_translation_ptr, (u32)cache_end_translation_ptr - (u32)cache_translation_ptr); \
                                                                              \
  return 0;                                                                   \
}                                                                             \
...
void init_translation_cache()
{
  /*
  mprotect(rom_translation_cache, sizeof(rom_translation_cache),
      PROT_READ | PROT_WRITE | PROT_EXEC);
  mprotect(ram_translation_cache, sizeof(ram_translation_cache),
      PROT_READ | PROT_WRITE | PROT_EXEC);
  mprotect(bios_translation_cache, sizeof(bios_translation_cache),
      PROT_READ | PROT_WRITE | PROT_EXEC);
  */
  flush_translation_cache_rom();
  flush_translation_cache_ram();
...
/*  printf("ram flush %d (pc %x), %x to %x, %x to %x\n",
  ...
  if(ewram_code_min != 0xFFFFFFFF)
  {
    ...
    else
    {
      ...
      memset(ewram + (ewram_code_min_page * 0x10000) + ewram_code_min_offset,
	    0, 0x8000 - ewram_code_min_offset);
      if (ewram_code_max_offset) {
