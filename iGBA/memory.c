...
backup_type_type backup_type_force = BACKUP_NONE;
...
flash_size_type flash_size_force = FLASH_SIZE_64KB;
  ...
  {                                                                           \
    ...
    case 0x08:                                                                \
	  if (rtc_state != RTC_DISABLED &&                                        \
          (address == 0x80000C4 || address == 0x80000C6 ||                    \
          address == 0x80000C8))                                              \
      {                                                                       \
		value = read_rtc(address & 0xFF);                                     \
		break;                                                                \
      }                                                                       \
	  /* fall through */                                                      \
    case 0x09:                                                                \
...
static u16 function_cc read_rtc(u32 address)
{
	return rtc_registers[(0xC4 - address) >> 1];
}
...
#define write_memory(type)                                                    \
  ...
  {                                                                           \
    case 0x02:                                                                \
      ...
      if (address##type(ewram, address - 0x8000))                             \
...
u32 load_backup(const char *name)
{
  ...
  backup_type = backup_type_force;
  flash_size = flash_size_force;
  sram_size = SRAM_SIZE_32KB;
  eeprom_size = EEPROM_512_BYTE;
...
void set_backup_type(backup_type_type type)
{
	backup_type_force = type;
}

void set_flash_size(flash_size_type type)
{
	flash_size_force = type;
}
...
s32 load_game_config(u8 *gamepak_title, u8 *gamepak_code, u8 *gamepak_maker)
{
  ...
  /* TODO */
  if (!strcmp(gamepak_title, "DRAGONBALL Z") &&
      !strcmp(gamepak_code, "ALGP") &&
      !strcmp(gamepak_maker, "70"))
	backup_type = BACKUP_EEPROM;

  sprintf(config_path, "%s", get_resource_path(CONFIG_FILENAME));
  ...
  config_file = fopen(config_path, "rb");
	...
	fprintf(stderr, "Loading game_config.txt!\n");
  if(config_file)
  {
	fprintf(stderr, "scanning game_config.txt!\n");
    ...
    while(fgets(current_line, 256, config_file))
    {
       ...
       != -1)
      {
		...
		fprintf(stderr, "game_config.txt game successfully read!\n");
        return 0;
      }
    }

	fprintf(stderr, "game_config.txt FAILED to find the game in the config!\n");
    fclose(config_file);
  }
...
u32 load_gamepak(const char *name)
{
  ...
  file_size = gamepak_ram_buffer_size;
  if (!load_archive(name, NULL, gamepak_rom, &file_size))
...
{
  int size = 0x4000;
  if (load_archive(name, "gba_bios.bin", bios_rom, &size))
    return size;
...
#define dma_write_ext(type, transfer_size)                                    \
  return_value |= write_memory##transfer_size(type##_ptr, read_value)         \
...
memory_savestate_builder(read_mem);
...
void load_state(const char *savestate_filename)
{
  ...
  int n = sizeof(savestate_write_buffer);

  if (!load_archive(savestate_filename, "GAMEBOID",
      savestate_write_buffer, &n))
  {
    n = 0;
	  ...
	  if (file_check_valid(savestate_file))
	  {
	    file_seek(savestate_file, (240 * 160 * 2) + sizeof(time_t), SEEK_SET);
		  n = file_read(savestate_file,
				savestate_write_buffer, sizeof(savestate_write_buffer));
		  n *= sizeof(savestate_write_buffer);
		  file_close(savestate_file);
	  }
  }
  if (n != sizeof(savestate_write_buffer))
  	return;
...
void save_state(const char* savestate_filename, const char* screenshot)
{
  FILE *savestate_file = NULL;

  write_mem_ptr = savestate_write_buffer;
  savestate_block(write_mem);
  save_archive(savestate_filename, "GAMEBOID",
  		savestate_write_buffer, sizeof(savestate_write_buffer));
}
