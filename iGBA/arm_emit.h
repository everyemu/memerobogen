...
#define write32(value)                                                        \
  *((u32 *)translation_ptr) = value;                                          \
  /*fprintf(stderr, "write32 0x%x translation_ptr 0x%x \n", *((u32 *)translation_ptr), translation_ptr); fflush(stderr);*/ \
  translation_ptr += 4                                                        \
...
static u32 imm_stores[4];
static u32 imm_rotations[4];

#define arm_load_imm_32bit(ireg, imm)                                         \
{                                                                             \
  u32 store_count = arm_disect_imm_32bit(imm, imm_stores, imm_rotations);     \
  u32 i;                                                                      \
                                                                              \
  ARM_MOV_REG_IMM(0, ireg, imm_stores[0], imm_rotations[0]);                  \
                                                                              \
  for(i = 1; i < store_count; i++)                                            \
  {                                                                           \
    ARM_ORR_REG_IMM(0, ireg, ireg, imm_stores[i], imm_rotations[i]);          \
  }                                                                           \
}                                                                             \
...
#define generate_block_extra_vars_arm()                                       \
  ...
  {                                                                           \
    ...
    generate_indirect_branch_no_cycle_update(dual_arm);                       \
                                                                              ...
                                                                              \
  u32 prepare_load_reg_arm(u32 scratch_reg, u32 reg_index)                        \
  {                                                                           \
    ...
    return reg_use;                                                           \
                                                                              ...
                                                                              \
  u32 prepare_load_reg_pc_arm(u32 scratch_reg, u32 reg_index, u32 pc_offset)      \
  {                                                                           \
    if(reg_index == 15)                                                       \
    {                                                                         \
      generate_load_pc(scratch_reg, g_pc + pc_offset);                          \
      return scratch_reg;                                                     \
    }                                                                         \
    return prepare_load_reg(scratch_reg, reg_index);                          \
  }                                                                           \
                                                                              \
  u32 prepare_store_reg_arm(u32 scratch_reg, u32 reg_index)                       \
  {                                                                           \
    u32 reg_use = arm_register_allocation[reg_index];                         \
    if(reg_use == mem_reg)                                                    \
      return scratch_reg;                                                     \
    ...
    return reg_use;                                                           \
                                                                              ...
                                                                              \
  void complete_store_reg_arm(u32 scratch_reg, u32 reg_index)                     \
  {                                                                           \
  ...
  {                                                                           \
    ...
    {                                                                         \
      complete_store_reg(scratch_reg, reg_index);                             \
                                                                              ...
                                                                              \
  void generate_load_reg_arm(u32 ireg, u32 reg_index)                             \
  {                                                                           \
    ...
    {                                                                         \
      ARM_LDR_IMM(0, ireg, reg_base, (reg_base_offset + (reg_index * 4)));    \
                                                                              ...
                                                                              \
  void generate_store_reg_arm(u32 ireg, u32 reg_index)                            \
  {                                                                           \
...
#define generate_block_extra_vars_thumb()                                     \
  u32 prepare_load_reg_thumb(u32 scratch_reg, u32 reg_index)                        \
  {                                                                           \
    ...
    return reg_use;                                                           \
                                                                              ...
                                                                              \
  u32 prepare_load_reg_pc_thumb(u32 scratch_reg, u32 reg_index, u32 pc_offset)      \
  {                                                                           \
    if(reg_index == 15)                                                       \
    {                                                                         \
      generate_load_pc(scratch_reg, g_pc + pc_offset);                          \
    ...
    return prepare_load_reg(scratch_reg, reg_index);                          \
                                                                              ...
                                                                              \
  u32 prepare_store_reg_thumb(u32 scratch_reg, u32 reg_index)                       \
  {                                                                           \
    u32 reg_use = thumb_register_allocation[reg_index];                       \
    if(reg_use == mem_reg)                                                    \
      return scratch_reg;                                                     \
    ...
    return reg_use;                                                           \
                                                                              ...
                                                                              \
  void complete_store_reg_thumb(u32 scratch_reg, u32 reg_index)                     \
  {                                                                           \
    ...
    {                                                                         \
      ARM_STR_IMM(0, scratch_reg, reg_base,                                   \
       (reg_base_offset + (reg_index * 4)));                                  \
                                                                              ...
                                                                              \
  void generate_load_reg_thumb(u32 ireg, u32 reg_index)                             \
  {                                                                           \
    ...
    {                                                                         \
      ARM_LDR_IMM(0, ireg, reg_base, (reg_base_offset + (reg_index * 4)));    \
                                                                              ...
                                                                              \
  void generate_store_reg_thumb(u32 ireg, u32 reg_index)                            \
  {                                                                           \
...
#define generate_load_reg_pc(ireg, reg_index, pc_offset)                      \
  if(reg_index == 15)                                                         \
  {                                                                           \
    generate_load_pc(ireg, g_pc + pc_offset);                                   \
  }                                                                           \
  else                                                                        \
  {                                                                           \
...
#define arm_block_memory_smc_init_load()                                      \

#define arm_block_memory_smc_init_store()                                     \
  generate_load_imm(reg_a2, 0, 0)                                             \

#define arm_block_memory_smc_final_load()                                     \

#define arm_block_memory_smc_final_store()                                    \
  generate_function_call(smc_check);                                          \
  write32((g_pc + 4))                                                           \
...
{                                                                             \
  ...
  arm_block_memory_smc_init_##access_type();                                  \
  ...
  arm_block_memory_smc_final_##access_type();                                 \
}                                                                             \
...
{                                                                             \
  thumb_decode_imm();                                                         \
  u32 __rd = prepare_store_reg(reg_rd, _rd);                                  \
  generate_load_pc(__rd, (((g_pc & ~2) + 4) + (imm * 4)));                      \
  complete_store_reg(__rd, _rd);                                              \
}                                                                             \
...
#define thumb_block_memory_smc_init_load()                                    \

#define thumb_block_memory_smc_init_store()                                   \
  generate_load_imm(reg_a2, 0, 0)                                             \

#define thumb_block_memory_smc_final_load()                                   \

#define thumb_block_memory_smc_final_store()                                  \
  generate_function_call(smc_check);                                          \
  write32((g_pc + 2))                                                           \
...
{                                                                             \
  ...
  thumb_block_memory_smc_init_##access_type();                                \
                                                                              \
  cycle_count += 2;                                                           \
                                                                              \
  for(i = 0; regs; i++)                                                       \
  ...
  thumb_block_memory_smc_final_##access_type();                               \
}                                                                             \
...
#define thumb_bl()                                                            \
  generate_update_pc(((g_pc + 2) | 0x01));                                      \
  generate_store_reg(reg_a0, REG_LR);                                         \
  generate_branch(thumb)                                                      \
