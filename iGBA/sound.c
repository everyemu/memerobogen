...
void sound_timer(fixed16_16 frequency_step, u32 channel)
{
  ...
  s16 current_sample = ds->fifo[ds->fifo_base] << 4;
...
void update_sound(void *userdata, u8 *stream, int length);
...
void init_sound()
{
  ...
  {
    fprintf(stderr, "Cannot init audio! \n");
    return;
  }
#endif
...
void update_sound(void *userdata, u8 *stream, int length)
{
  ...
  if(!global_enable_audio) { memset(stream, 0, 2048);  return; }
  ...
  //render_audio(stream, (sample_length >> 1) & ~1);
}
...
sound_savestate_builder(read_mem);
