...
inline void WRITE16LE(u16 *address, u16 value)
{
	if (!(((u32) address) & 1))
		*address = value;
	else {
...
extern "C" void _flush_translation_cache_ram();
extern "C" void _flush_translation_cache_rom();

#define CHEAT_PATCH_ROM_16BIT(a,v) { \
  WRITE16LE(((u16 *)&gamepak_rom[(a) & 0x1ffffff]), v); \
  _flush_translation_cache_rom(); \
...
#define cheats_write_memory(size, address, value) \
	if (read_memory##size((address)) != (value)) { \
		smc_flag |= (write_memory##size((address), (value)) & CPU_ALERT_SMC); \
	}
...
int cheatsCheckKeys(u32 keys, u32 extended)
{
  ...
  for(int i = 0; i < cheatsNumber; i++) {
    ...
    switch(cheatsList[i].size) {
    case INT_8_BIT_WRITE:
      cheats_write_memory(8, cheatsList[i].address, cheatsList[i].value);
      break;
    case INT_16_BIT_WRITE:
      cheats_write_memory(16, cheatsList[i].address, cheatsList[i].value);
      break;
    case INT_32_BIT_WRITE:
      cheats_write_memory(32, cheatsList[i].address, cheatsList[i].value);
      break;
    case GSA_16_BIT_ROM_PATCH:
      if((cheatsList[i].status & 1) == 0) {
        if(read_memory16(cheatsList[i].address) != cheatsList[i].value) {
          cheatsList[i].oldValue = read_memory16(cheatsList[i].address);
    ...
    case CBA_SLIDE_CODE:
      {
        ...
        if(i < cheatsNumber) {
          ...
          for(int x = 0; x < count; x++) {
            cheats_write_memory(16, address, value);
            address += inc;
            value += vinc;
          }
        }
    ...
    case CBA_IF_FALSE:
      ...
      break;
    case CBA_AND:
      cheats_write_memory(16, cheatsList[i].address,
                       read_memory16(cheatsList[i].address) &
                       cheatsList[i].value);
      break;
    case GSA_8_BIT_GS_WRITE2:
      i++;
      if(i < cheatsNumber) {
        if(extended & 1) {
          cheats_write_memory(8, cheatsList[i-1].value, cheatsList[i].address);
        }
      }
      break;
    case GSA_16_BIT_GS_WRITE2:
      ...
      if(i < cheatsNumber) {
        if(extended & 1) {
          cheats_write_memory(16, cheatsList[i-1].value, cheatsList[i].address);
        }
      }
      break;
    case GSA_32_BIT_GS_WRITE2:
      ...
      if(i < cheatsNumber) {
        if(extended & 1) {
          cheats_write_memory(32, cheatsList[i-1].value, cheatsList[i].address);
        }
      }
      break;
    case GSA_16_BIT_ROM_PATCH2:
      ...
      if(i < cheatsNumber) {
        if((cheatsList[i-1].status & 1) == 0) {
          ...
          if(read_memory16(addr) != (cheatsList[i].address & 0xFFFF)) {
            cheatsList[i-1].oldValue = read_memory16(addr);
    ...
    case GSA_8_BIT_SLIDE:
      ...
      if(i < cheatsNumber) {
        ...
        while(count > 0) {
          cheats_write_memory(8, addr, value);
          value += vinc;
          addr += ainc;
          count--;
        }
    ...
    case GSA_16_BIT_SLIDE:
      ...
      if(i < cheatsNumber) {
        ...
        while(count > 0) {
          cheats_write_memory(16, addr, value);
          value += vinc;
          addr += ainc;
          count--;
        }
    ...
    case GSA_32_BIT_SLIDE:
      ...
      if(i < cheatsNumber) {
        ...
        while(count > 0) {
          cheats_write_memory(32, addr, value);
          value += vinc;
          addr += ainc;
          count--;
        }
      ...
      break;
    case GSA_8_BIT_IF_TRUE:
      if(read_memory8(cheatsList[i].address) != cheatsList[i].value) {
        i++;
      }
      break;
    case GSA_32_BIT_IF_TRUE:
      if(read_memory32(cheatsList[i].address) != cheatsList[i].value) {
        i++;
      }
      break;
    case GSA_8_BIT_IF_FALSE:
    ...
    case GSA_SLOWDOWN:
      ...
      if((cheatsList[i].status & 4) && !(extended & 1))
        cheatsList[i].status ^= 1;
      if(extended & 1)
    ...
    case CBA_SUPER:
      {
        ...
        for(int x = 0; x < count; x++) {
          ...
          cheats_write_memory(8, address, b);
          address++;
          if(x && !res)
            i++;
        }
...
{
	for (int i = 0; i < cheatsNumber; i++) {
    	if(strcmp(cheatsList[i].codestring, code) == 0)
			return i;
	}
	return -1;
}
               ...
               int size)
{
  if(cheatsNumber < 100) {
    ...
    case INT_8_BIT_WRITE:
      cheatsList[x].oldValue = read_memory8(address);
      break;
    case INT_16_BIT_WRITE:
      cheatsList[x].oldValue = read_memory16(address);
      break;
    case INT_32_BIT_WRITE:
      cheatsList[x].oldValue = read_memory32(address);
      break;
    }
    cheatsNumber++;
  }
...
void cheatsDelete(int number, BOOL restore)
{
  ...
  if(number < cheatsNumber && number >= 0) {
    ...
    if(restore) {
      switch(cheatsList[x].size) {
      case INT_8_BIT_WRITE:
        cheats_write_memory(8, cheatsList[x].address, (u8)cheatsList[x].oldValue);
        break;
      case INT_16_BIT_WRITE:
        cheats_write_memory(16, cheatsList[x].address, (u16)cheatsList[x].oldValue);
        break;
      case INT_32_BIT_WRITE:
        cheats_write_memory(32, cheatsList[x].address, cheatsList[x].oldValue);
        break;
      case GSA_16_BIT_ROM_PATCH:
...
BOOL cheatsVerifyCheatCode(const char *code)
{
  int len = strlen(code);
  ...
  if(len == 17)
    type = 2;
  cheatsAdd(code, address, value, type, type);
  return TRUE;
}

void cheatsAddCheatCode(const char *code)
{
  cheatsVerifyCheatCode(code);
}
...
BOOL cheatsAddGSACode(const char *codeStr)
{
  char code[17];
  BOOL v3 = FALSE;
  ...
  if (len == 16)
  {
    memcpy(code, codeStr, len);
  }
  else if (len == 17 && codeStr[8] == ' ')
  {
    memcpy(code, codeStr, 8);
    memcpy(code + 8, codeStr + 9, 8);
    v3 = TRUE;
  }
  else
  {
  ...
  for(i = 0; i < 16; i++)
  {
    ...
    {
      // wrong cheat
      // LOGD("Invalid GSA code. Format is XXXXXXXXYYYYYYYY");
      return FALSE;
    }
  }
  ...
  if(value == 0x1DC0DE) {
    u32 gamecode = *(u32 *)&gamepak_rom[0xac];
    if(gamecode != address) {
      char buffer[5];
      *((u32 *)buffer) = address;
      buffer[4] = 0;
      char buffer2[5];
      *((u32 *)buffer2) = *(u32 *)&gamepak_rom[0xac];
      buffer2[4] = 0;
      // LOGD("Warning: cheats are for game %s. Current game is %s.\nCodes may not work correctly.", buffer, buffer2);
    }
    cheatsAdd(codeStr, address & 0x0FFFFFFF, value, v3 ? 257 : 256, 
              UNKNOWN_CODE);
    return TRUE;
  }
  if(isMultilineWithData(cheatsNumber-1)) {
    cheatsAdd(codeStr, address, value, v3 ? 257 : 256, UNKNOWN_CODE);
    return TRUE;
  }
  if(v3) {
    ...
    case 0x00:
      if(address == 0) {
        ...
        switch(type) {
        case 0x04:
          cheatsAdd(codeStr, 0, value & 0x00FFFFFF, 257, GSA_SLOWDOWN);
          break;
        case 0x08:
          cheatsAdd(codeStr, 0, addr, 257, GSA_8_BIT_GS_WRITE2);
          break;
        case 0x09:
          cheatsAdd(codeStr, 0, addr, 257, GSA_16_BIT_GS_WRITE2);
          break;
        case 0x0a:
          cheatsAdd(codeStr, 0, addr, 257, GSA_32_BIT_GS_WRITE2);
          break;
        case 0x0c:
        case 0x0d:
        case 0x0e:
        case 0x0f:
          cheatsAdd(codeStr, 0, value & 0x00FFFFFF, 257, GSA_16_BIT_ROM_PATCH2);
          break;
        case 0x40:
          cheatsAdd(codeStr, 0, addr, 257, GSA_8_BIT_SLIDE);
          break;
        case 0x41:
          cheatsAdd(codeStr, 0, addr, 257, GSA_16_BIT_SLIDE);
          break;
        case 0x42:
          cheatsAdd(codeStr, 0, addr, 257, GSA_32_BIT_SLIDE);
          break;
        default:
          cheatsAdd(codeStr, address, value, 257, UNKNOWN_CODE);
          break;
        }
      } else
        cheatsAdd(codeStr, addr, value, 257, GSA_8_BIT_FILL);
      break;
    case 0x01:
      cheatsAdd(codeStr, addr, value, 257, GSA_16_BIT_FILL);
      break;
    case 0x02:
      cheatsAdd(codeStr, addr, value, 257, INT_32_BIT_WRITE);
      break;
    case 0x04:
      cheatsAdd(codeStr, addr, value, 257, GSA_8_BIT_IF_TRUE);
      break;
    case 0x05:
      cheatsAdd(codeStr, addr, value, 257, CBA_IF_TRUE);
      break;
    case 0x06:
      cheatsAdd(codeStr, addr, value, 257, GSA_32_BIT_IF_TRUE);
      break;
    case 0x08:
      cheatsAdd(codeStr, addr, value, 257, GSA_8_BIT_IF_FALSE);
      break;
    case 0x09:
      cheatsAdd(codeStr, addr, value, 257, CBA_IF_FALSE);
      break;
    case 0x0a:
      cheatsAdd(codeStr, addr, value, 257, GSA_32_BIT_IF_FALSE);
      break;
    case 0x24:
      cheatsAdd(codeStr, addr, value, 257, GSA_8_BIT_IF_TRUE2);
      break;
    case 0x25:
      cheatsAdd(codeStr, addr, value, 257, GSA_16_BIT_IF_TRUE2);
      break;
    case 0x26:
      cheatsAdd(codeStr, addr, value, 257, GSA_32_BIT_IF_TRUE2);
      break;
    case 0x28:
      cheatsAdd(codeStr, addr, value, 257, GSA_8_BIT_IF_FALSE2);
      break;
    case 0x29:
      cheatsAdd(codeStr, addr, value, 257, GSA_16_BIT_IF_FALSE2);
      break;
    case 0x2a:
      cheatsAdd(codeStr, addr, value, 257, GSA_32_BIT_IF_FALSE2);
      break;
    default:
      cheatsAdd(codeStr, address, value, 257, UNKNOWN_CODE);
      break;
    ...
    case 1:
    case 2:
      cheatsAdd(codeStr, address & 0x0FFFFFFF, value, 256, type);
      break;
    case 6:
      address <<= 1;
      type = (address >> 28) & 15;
      if(type == 0x0c) {
        cheatsAdd(codeStr, address & 0x0FFFFFFF, value, 256, 
                  GSA_16_BIT_ROM_PATCH);
        break;
      }
      // unsupported code
      cheatsAdd(codeStr, address, value, 256, 
                UNKNOWN_CODE);
      break;
    case 8:
      switch((address >> 20) & 15) {
      case 1:
        cheatsAdd(codeStr, address & 0x0F0FFFFF, value, 256, 
                  GSA_8_BIT_GS_WRITE);
        break;
      case 2:
        cheatsAdd(codeStr, address & 0x0F0FFFFF, value, 256, 
      ...
      case 3:
        cheatsAdd(codeStr, address & 0x0F0FFFFF, value, 256, 
                  GSA_32_BIT_GS_WRITE);
      case 15:
        cheatsAdd(codeStr, 0, value & 0xFF00, 256, GSA_SLOWDOWN);
        break;
      default:
        // unsupported code
        cheatsAdd(codeStr, address, value, 256, 
...
BOOL cheatsAddCBACode(const char *code)
{
  if(strlen(code) != 13) {
    // wrong cheat
    // LOGD("Invalid CBA code. Format is XXXXXXXX YYYY.");
    return FALSE;
  }
  ...
  u8 array[8] /* __attribute__ ((aligned(4))) */ = {
    address & 255,
    (address >> 8) & 255,
    (address >> 16) & 255,
    (address >> 24) & 255,
  ...
  if(cheatsCBAGetCount() == 0 &&
    ...
    cheatsAdd(code, address & 0x0FFFFFFF, value, 512, UNKNOWN_CODE);
  } else {
    ...
    if(isMultilineWithData(cheatsNumber-1)) {
      cheatsAdd(code, address, value, 512, UNKNOWN_CODE);
      return TRUE;
    }
    ...
    case 0x00:
      {
        if(!cheatsCBATableGenerated)
          cheatsCBAGenTable();
        u32 crc = cheatsCBACalcCRC(gamepak_rom, 0x10000);
        if(crc != address) {
          // LOGD("Warning: Codes seem to be for a different game.\nCodes may not work correctly.");
        }
        cheatsAdd(code, address & 0x0FFFFFFF, value, 512, 
                  UNKNOWN_CODE);
      }
      break;
    case 0x02:
    ...
    default:
      // unsupported code
      cheatsAdd(code, address & 0xFFFFFFFF, value, 512, 
                UNKNOWN_CODE);
      break;
    }
  }
